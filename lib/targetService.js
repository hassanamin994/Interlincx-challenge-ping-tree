...
async function getUsableTarget (query) {
  ...
  const intersectionKey = `target:accept.query_inter:${searchkeys.join(':')}`
  ...
  for (const memberId of members) {
    ...
    let targetUsedAcceptsPerDay = parseFloat(
      await redisClient.hmget(targetUsedAcceptsPerDayHMKey, target.id)
    )
    // if expired, re-create for another 24 hours
    if (Number.isNaN(targetUsedAcceptsPerDay)) {
      await redisClient.hmset(targetUsedAcceptsPerDayHMKey, target.id, 0, 'EX', 24 * 60 * 60)
      targetUsedAcceptsPerDay = 0
    }
